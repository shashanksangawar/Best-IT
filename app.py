...
def full_info():
    ...
    if info_type == 'Available':
        return full_info_available_working(serial_no=edit_serial)
    if info_type == 'Sold':
        return full_info_sold_working(serial_no=edit_serial)

@app.route("/update_backend", methods=["GET", "POST"])
def update_backend():
    form = request.form
    try:
        serial_no = edit_serial
        message, statuscode = update_items(serial_no=serial_no, request_json=form)
